...
def _upload(client, name="ops.txt", content=b"fresh return policy content"):
    kb = client.post("/api/kb", json={"name": "运营库"}).json()
    r = client.post(f"/api/kb/{kb['id']}/documents",
                    files={"file": (name, content, "text/plain")})
    return kb, r


def test_kb_create_and_list(app_client):
    r = app_client.post("/api/kb", json={"name": "产品手册库", "description": "SKU规范"})
    assert r.status_code == 201
    body = r.json()
    assert body["name"] == "产品手册库" and body["id"] > 0
    listed = app_client.get("/api/kb").json()
...
def test_upload_txt_creates_ready_doc_and_chunks(app_client):
    ...
    chunks = app_client.get(f"/api/documents/{doc['id']}/chunks").json()
...
def test_upload_rejects_unsupported_ext(app_client):
    kb = app_client.post("/api/kb", json={"name": "k"}).json()
    r = app_client.post(f"/api/kb/{kb['id']}/documents",
...
def test_document_status_visible_and_reprocess(app_client):
    ...
    doc = doc.json()
    got = app_client.get(f"/api/documents/{doc['id']}").json()
    assert got["status"] == "ready"
    app_client.patch(f"/api/documents/{doc['id']}", json={"status": "failed",
                                                         "error": "模拟解析失败"})
    r = app_client.post(f"/api/documents/{doc['id']}/reprocess")
...
def test_retrieve_endpoint(app_client):
    ...
    hits = app_client.post("/api/retrieve", json={
...
def test_chat_returns_answer_citations_and_persists_history(app_client, chat_calls):
    ...
    q = "退货要多久响应？"
    resp = app_client.post("/api/chat", json={"question": q, "kb_ids": [kb["id"]]}).json()
    ...
    msgs = app_client.get(f"/api/conversations/{conv_id}/messages").json()
    assert [m["role"] for m in msgs] == ["user", "assistant"]
    assert msgs[0]["content"][0] == {"type": "text", "text": q}
    assert msgs[1]["citations"][0]["chunk_id"] > 0
    assert app_client.get("/api/conversations").json()[0]["id"] == conv_id
...
def test_404_for_unknown_resources(app_client):
    assert app_client.get("/api/documents/999").status_code == 404
    assert app_client.get("/api/conversations/999/messages").status_code == 404
    kb = app_client.post("/api/kb", json={"name": "x"}).json()
    assert app_client.post(f"/api/kb/{kb['id']}/documents",
...
def test_chat_writes_usage_record(app_client, db: Session):
    ...
    app_client.post("/api/chat", json={"question": "生鲜 退货 政策", "kb_ids": [kb["id"]]})
